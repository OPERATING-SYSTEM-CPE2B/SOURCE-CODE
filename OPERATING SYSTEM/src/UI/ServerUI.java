...
public class ServerUI extends JFrame implements ActionListener {	
	...
	public ServerUI() {
		...
		setBounds(150, 150, 720, 480);
		contentPane = new JPanel();
		contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));
		contentPane.setBackground(new Color(21,32,43));
		setContentPane(contentPane);
		contentPane.setLayout(new BorderLayout(0, 0));

		lblChatServer = new JLabel("USTP CHAT SERVER");
		...
		btnStart.setBackground(new Color(21,32,43));

		JScrollPane scrollPane = new JScrollPane();
		contentPane.add(scrollPane, BorderLayout.CENTER);

		txtAreaLogs = new JTextArea();
		txtAreaLogs.setBackground(new Color(208,181,41));
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == btnStart) {
			if(btnStart.getText().equals("START")) {
				...
				addToLogs("USTP Chat Server stopped...");
				exit = true;
	...
	public void refreshUIComponents() {
		lblChatServer.setText("CHAT SERVER" + (!exit ? ": "+PORT:": OFFLINE"));
	}
	...
	private static class ServerHandler implements Runnable{
		...
		public void run() {
			try {
				...
				addToLogs("USTP Server started on port: " + PORT);
